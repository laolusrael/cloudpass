<script lang="ts">
	...
	async function createSnapshot() {
		...
		error = null;
		try {
			const response = await api.createSnapshot(name, {
				name: snapshotName,
...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-4">
			...
			{#if instanceState}
				<span
					class="px-2 py-1 text-xs font-medium rounded {instanceState.state === 'Running'
	...
	{#snippet actions(snapshot: Snapshot)}
		<div class="flex gap-2 justify-end">
			...
			<Button variant="danger" size="sm" onclick={() => handleDelete(snapshot.name)}>Delete</Button>
		</div>
	{/snippet}
		...
		<Table
			...
			columns={[
				{ key: 'name', header: 'Name', sortable: true },
				{
					...
					header: 'Created',
					sortable: true,
					render: (s) => (s.created_at ? new Date(s.created_at).toLocaleString() : '-')
				},
				{ key: 'comment', header: 'Comment', sortable: true, render: (s) => s.comment || '-' },
				{
					...
					header: 'Disk Size',
					sortable: true,
					render: (s) => formatSize(s.disk_size)
				}
			]}
			{actions}
		/>
...
<Modal open={showStopModal} title="Instance is Running" onclose={() => (showStopModal = false)}>
	<div class="space-y-4">
		...
		<p class="text-gray-600">
			Snapshots can only be created on stopped instances. Would you like to stop the instance and
			create a snapshot?
		</p>
		<div class="bg-yellow-50 border border-yellow-200 rounded p-3 text-sm text-yellow-800">
			<strong>Note:</strong> The instance will be stopped, snapshot will be created, and then the instance
			will remain stopped. You can start it again after the snapshot is created.
		</div>
	</div>

	{#snippet footer()}
		<div class="flex gap-2 justify-end">
			<Button variant="secondary" onclick={() => (showStopModal = false)}>Cancel</Button>
...
>
	...
	{#snippet footer()}
		<Button variant="primary" onclick={() => (showSuccessModal = false)}>OK</Button>
	{/snippet}
</Modal>
